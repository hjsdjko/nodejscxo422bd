...
import JiaoshiModel from '../models/JiaoshiModel'
...
export default ({ config, db }) => {
	...
	api.all('/login', async (req, res) => {
		...
		try {
			...
			let userinfo = await JiaoshiModel.findOne({ where: { jiaoshigonghao: req.query.username, mima: pwd } })
			if (userinfo === null) {
			...
			const token = jwt.sign(
				{
					...
					username: userinfo.dataValues.jiaoshigonghao,
					role: '教师',
                    loginUserColumn: 'jiaoshigonghao',
					jiaoshigonghao:userinfo.dataValues.jiaoshigonghao,
					mima:userinfo.dataValues.mima,
					jiaoshixingming:userinfo.dataValues.jiaoshixingming,
					xingbie:userinfo.dataValues.xingbie,
					zhaopian:userinfo.dataValues.zhaopian,
					zhicheng:userinfo.dataValues.zhicheng,
			...
			userinfo.dataValues.role = '教师'
			userinfo.dataValues.loginUserColumn = 'jiaoshigonghao'
			userinfo.dataValues.tableName = 'jiaoshi'
	...
	api.post('/register', async (req, res) => {
		...
		try {
			...
			const userinfo = await JiaoshiModel.create(req.body)
	...
	api.all('/session', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await JiaoshiModel.findOne({ where: { id:jwt.decode(req.headers.token).id} }))
		} catch(err) {
	...
	api.all('/resetPass', async (req, res) => {
		...
		try {
    		...
    		let pwd = '123456'
			await JiaoshiModel.update({
				mima: pwd
			}, {
				where: {
				  jiaoshigonghao: req.query.username
				}
			})
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let jiaoshixingming = req.query.jiaoshixingming
			if (jiaoshixingming) {
			...
			let result = await JiaoshiModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await JiaoshiModel.findAll()
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await JiaoshiModel.findOne({where:dictionary})
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			let jiaoshigonghaocount = await JiaoshiModel.count({
				where: {
					jiaoshigonghao: req.body["jiaoshigonghao"]
				}
			})
            if (jiaoshigonghaocount > 0) {
				toRes.session(res, -1, '教师工号已存在')
            }
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await JiaoshiModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await JiaoshiModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM jiaoshi WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM jiaoshi WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM jiaoshi WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM jiaoshi WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM jiaoshi WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM jiaoshi WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM jiaoshi " + where + " GROUP BY " + xColumnName + " DESC"
