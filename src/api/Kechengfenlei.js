...
export default ({ config, db }) => {
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await KechengfenleiModel.findOne({where:dictionary})
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			let kechengfenleicount = await KechengfenleiModel.count({
				where: {
					kechengfenlei: req.body["kechengfenlei"]
				}
			})
            if (kechengfenleicount > 0) {
				toRes.session(res, -1, '课程分类已存在')
            }
